...
//	REVISION:   w
//	CHECKSUM:	0x320a (MPLABX ver 3.15 and XC16 ver 1.26)
//	DATA(RAM)MEM:	7874/30720   26%
//	PGM(FLASH)MEM:  147300/261888 56%
...
//                                  Need to reload Start & Stop logging times so they repeat on a daily basis
//
//
//
//				VW GAGE TYPES				
...
//#include "LC8004extFRAM_e.h"                              
//#include "LC8004main_w.h"
...
void __attribute__((__interrupt__)) _INT1Interrupt(void)                        //This is the RTC ISR when time to read occurs  REV B
{
    ...
    if (validRTC) //if valid interrupt
    {
        ...
        if (PORT_CONTROL.flags.Alarm1Enabled && (intSource == 0x01 | intSource == 0x03)) //Alarm 1 (NO NEED TO TEST Alarm1Enabled flag)
        {
            ...
            MUX_RESET = 0; //set the MUX_RESET line low
            disableAlarm(Alarm1);
            //intSource = readClock(0x0F);                                        //get the RTC Alarm flags		TEST REV W
            //intSource &= 0x03;                                                  //strip off bits 7-2    TEST REV W    
            //Nop();            
    ...
    //*********************TEST REV W:**************************************
        ...
        if (PORT_CONTROL.flags.Alarm2Enabled && (intSource == 0x02 | intSource == 0x03)) //Alarm 2   (NO NEED TO TEST Alarm2Enabled flag)
        {
            ...
            //intSource = readClock(0x0F);                                        //get the RTC Alarm flags		TEST REV W
            //intSource &= 0x03;                                                  //strip off bits 7-2    TEST REV W    
            //Nop();                                                              //check that correct flags are set  TEST REV W

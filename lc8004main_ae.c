...
//	REVISION:   ae
//	CHECKSUM:	0x795a (MPLABX ver 3.15 and XC16 ver 1.26)
//	DATA(RAM)MEM:	8286/30720   27%
//	PGM(FLASH)MEM:  151581/261888 58%
...
//	Z:\8004\LC8004main_ae.c (main source: x is revision level)
...
//***************************TEST FOR 3V or 12V BATTERY*****************************************************************
    ...
    //if(DISPLAY_CONTROL.flags.BT)                                                //Enable Bluetooth if previously enabled    REV AD
    //    enableBT();
...
float getFrequency(void)
{
	...
	for(i=0;i<loopsTotal;i++)
	{
		...
		//OpenTimer1(T1_ON & T1_GATE_OFF & T1_PS_1_64 & T1_SOURCE_INT, 0x7900);	//setup Timer 1	 for 300mS timeout REM REV AE
        OpenTimer1(T1_ON & T1_GATE_OFF & T1_PS_1_256 & T1_SOURCE_INT, 0x7900);	//setup Timer 1	 for 300mS timeout REV AE
		//Synchronize to VW:
...
void take_One_Complete_Reading(unsigned char store) 
{
    ...
    //REV Z:
    if (((!USB_PWR | _232) && DISPLAY_CONTROL.flags.PS12V && TEST12V < bat_12MIN) |   //if 12V battery < minimum V or 3V battery < minimum V  
            ((!USB_PWR | _232) && !DISPLAY_CONTROL.flags.PS12V && batteryVoltage < bat_3MIN)) {
        disableINT1(); //stop logging and shutdown
        disableAlarm(Alarm1);
        shutdown();
    }    

...
//	REVISION:   n
//	CHECKSUM:	0x05a1 (MPLABX ver 3.15 and XC16 ver 1.26)
//	DATA(RAM)MEM:	7958/30720   26%
//	PGM(FLASH)MEM:  141906/261888 54%
            ...
            (channel>=17&&(MUX4_ENABLE.mflags.mux16_4==1)) |        //16 channel
                ...
                case capM:
                    ...
                    if (buffer[1] == capX && buffer[2] == one && buffer[3] == cr) //Select single channel VW Datalogger
                    {
                        ...
                        MUX_ENABLE1_16.MUXen1_16=0;                             //clear the mux enable flags REV K
...
void configTimers(void)                                                         //REV M
{
    ...
    //Configure Timer7 as 16 bit counter with 1:8 prescaler and T7CK as input
    T7CONbits.TON=0;                                                            //make sure Timer 7 is off
    T7CONbits.TCKPS=1;                                                          //setup for 1:8 prescale  TEST REM REV M
    //T7CONbits.TCKPS=2;                                                          //setup for 1:64 prescale    TEST REV M
...
void configUARTnormal(void) {
    ...
    //SRbits.IPL = 0; //Set CPU Interrupt priority to 0                         //REM REV D
    SRbits.IPL=5;   //Set CPU Interrupt priority to 5                           //REV D
...
void displayReading(int ch, unsigned long outputPosition) //display readings stored in external FRAM at FRAMaddress
{
    ...
    if (!LC2CONTROL2.flags2.d) //VER 6.0.2
    {
        ...
        Thermtype=0;                                                            //set to read standard thermistor	REV J
        intThermProcessed=V_HT2C(intThermRaw,Thermtype);                        //convert to degrees C 	REV J
        //Thermtype=read_Int_FRAM(Thermtypeaddress);                              //restore thermistor type	REM REV J
...
void enableINT1(void)
{
    ...
    //ConfigINT1(FALLING_EDGE_INT & EXT_INT_PRI_7 & EXT_INT_ENABLE & GLOBAL_INT_ENABLE);    REM REV N
    ConfigINT1(FALLING_EDGE_INT & EXT_INT_PRI_6 & EXT_INT_ENABLE & GLOBAL_INT_ENABLE);  //Set IP to 3   REV N
    Nop();
}
...
float getFrequency(void)
{
	...
	for(i=0;i<loopsTotal;i++)
	{
		...
		//VWcountMSW=0;                                                           //clear the counter MSW register  REM REV M
		VWcountLSW=0;                                                           //clear the counter LSW register	
		//PR2=0xFFFF;                                                             //load PR2 with max count
		//PR3=0xFFFF;                                                             //load PR3 with max count
		...
		//T2CONbits.TCS=1;                                                      //set to count external T2CKI clocks
        T7CONbits.TCS=1;                                                        //set to count external T7CKI clocks
		IEC1bits.T5IE=1;                                                        //enable 512mS interrupt

		//OpenTimer1(T1_ON & T1_GATE_OFF & T1_PS_1_8 & T1_SOURCE_INT, 0xFFFF);		//setup Timer 1	 REM VER 5.8.1.1
		//OpenTimer1(T1_ON & T1_GATE_OFF & T1_PS_1_64 & T1_SOURCE_INT, 0x7900);	//setup Timer 1	 for 256mS timerout TEST REM REV M
		//T2CONbits.TON=1;							//start VW100 counter	
		...
		while(!VW100 && !IFS0bits.T1IF);                                        //wait while VW(100) is low
        //INTCON1bits.NSTDIS=1;                                                   //disable interrupt nesting TEST REM REV N
        IPC7bits.T5IP=7;                                                        //set INT5 as highest priority
		T7CONbits.TON=1;                                                        //start VW100 counter	
		if(!IFS0bits.T1IF)                                                      //Capture frequency				
		{
			T4CONbits.TON=1;                                                    //start 512mS timer	
            testPoint(1,1);
			//while(!IFS0bits.T1IF && CaptureFlag==0)                           TEST REM REV N
            while(CaptureFlag==0)                                               //TEST REV N
			{	
				Nop();                                                          //wait for 512mS to expire
			}
            testPoint(1,2);
            
			if(IFS0bits.T1IF)
				return 0;                                                       //timeout waiting for VW_100
			//T2CONbits.TON=0;                                                  //shut off counter
            T7CONbits.TON=0;                                                    //shut off counter
			T4CONbits.TON=0;                                                    //shut off timer
			CaptureFlag=0;                                                      //Reset captureFlag		
			//VWcount=VWcountMSW;                                                 //load 16 bit VWcountMSW into 32 bit VWcount
			//VWcount=(VWcountMSW*65536.0)+VWcountLSW;
            VWcount=VWcountLSW*8;                                               //compensate for 1:8 prescaler    TEST REM REV M
            //VWcount=VWcountLSW*64;                                               //compensate for 1:64 prescaler  TEST REV M
			//frequency=(VWcount/mS512)*10.0;                                     //convert to frequency	REM REV M
	...
	//need to reconfigure T5 here:
	shutdownTimer(TimeOut);                                                     //Reset 15S timer	VER 5.8.1.1	
    disableTimers();
	return frequency;
}
...
void loadDefaults(void) 
{
    ...
    //REV K:
    MUX_ENABLE1_16.MUXen1_16=0;                                                 //disable all channels except CH1
    MUX_ENABLE1_16.e1flags.CH1=1;
    write_Int_FRAM(MUX_ENABLE1_16flagsaddress,MUX_ENABLE1_16.MUXen1_16);        //store flag in FRAM
    
    MUX_ENABLE17_32.MUXen17_32=0;                                                 //disable all channels
    write_Int_FRAM(MUX_ENABLE17_32flagsaddress,MUX_ENABLE17_32.MUXen17_32);                        
...
void setup(void)
{
    ...
    IPC7bits.T5IP=4;                                                            //Set TMR5 IP to 4  REV N
...
void shutdown(void) 
{
    ...
    //INTCON1bits.NSTDIS = 1; //disable interrupt nesting                       //TEST REV D
...
void storeTempChannelReading(int ch)                                            //Store 'X' command readings to FRAM
{
            ...
            MUX4_ENABLE.mflags.mux16_4 == VW16) 
    {
        ...
        TempFRAMaddress = (XmemStart+0xE);// + (6 * (ch - 1));                     //compute memory location for VW reading  TEST REV M
...
unsigned int take_analog_reading(unsigned char gt)                                       //REV K Change u.c. gageType to u,c, gt & and return unsigned int
{
    ...
    if (gt >= 85 && gt <= 97)                                                   //REV K
    {
        ...
        //AD1CON3 = 0x080F;                                                       //Tad=16*Tcy, Sample time = 8*Tad
        ...
        AD1CON3bits.SAMC=0x1F;                                                  //Autosample time = 31 Tad
        ...
        //testPoint(1,1);                                                         //TEST REV K
        
        AD1CON1bits.ADON = 1;                                                   //turn ADC on	
        delay(2000);                                                            //delay 136uS for settling  REV J

        //REM REV J:
        /*
        IFS0bits.AD1IF=0;                                                       //clear the interrupt flag  FEV J
        ...
        */
        ...
        {
           //testPoint(1,1);                                                      //TEST REV K
           AD1CON1bits.ADON=1;                                                //TEST REM REV K
           while(!IFS0bits.AD1IF);                                            //TEST REM REV K
           IFS0bits.AD1IF=0;                                                  //TEST REM REV K
           analog=analog+*ADC16Ptr;
           AD1CON1bits.ADON=0;                                                //TEST REM REV K
        }
...
//	VWcountLSW=TMR2;										//read the least significant word
//	VWcountMSW=TMR3HLD;										//read the most significant word
//	CaptureFlag=1;											//set the capture flag	
//}

...
//	REVISION:   bg
//	CHECKSUM:	0x5e97 (MPLABX ver 3.15 and XC16 ver 1.26)
//	DATA(RAM)MEM:	8634/30720   28%
//	PGM(FLASH)MEM:  149364/261888 57%
...
//	Z:\8004\LC8004main_bg.c (main source: x is revision level)
//	Z:\8004\LC8004delay_b.c
//	Z:\8004\LC8004extFRAM_h.c
//  Z:\8004\AD5241a.c
//  Z:\8004\MODBUSd.c                                                           REV BA

//	Header Files:
//#include "p33FJ256GP710A.h"
//#include "LC8004extFRAM_h.h"                              
//#include "LC8004main_bg.h"
//#include "LC8004delay_b.h"
//#include "AD5241a.h"
//#include "MODBUSd.h"                                                          REV BA
    ...
    while (1) //infinite loop
    {
        ...
        if(DISPLAY_CONTROL.flags.Shutdown | IFS3bits.T9IF)                                      //REV D
        {
            ...
            //{
                shutdown();
            //}
            //shutdownTimer(TimeOut);                                             //REM REV Z			
        }
                ...
                case 2:
                    ...
                    if (!LC2CONTROL.flags.LoggingStartTime && !LC2CONTROL2.flags2.SetStopTime && !PORT_CONTROL.flags.SetAlarm2Time && !PORT_CONTROL.flags.SetAlarm2StopTime) {
                        ...
                        if (!LC2CONTROL2.flags2.SetStopTime && !PORT_CONTROL.flags.SetAlarm2Time)//if logging stop time is not being extracted from buffer
                        {
                            ...
                            if(DISPLAY_CONTROL.flags.firstTime)                 //REV W
                            {
                                write_Int_FRAM(startMinutesaddress,RTCminutes); //store in FRAM     REV W
...
void CMDcomm(void) 
{
    ...
    while (!IFS3bits.T9IF) //enable COM for Timeout period
    {
        ...
        if (!LC2CONTROL.flags.NetEnabled && !DISPLAY_CONTROL.flags.Shutup && !IFS3bits.T9IF)
        {
            putsUART1(Hello);
            while (BusyUART1());
            prompt();
            ...
            switch (buffer[0]) 
            {
                ...
                case capB:
                    ...
                    if (buffer[1]==capT && buffer[2] == capD && buffer[3] == cr) //BTD<CR> received REV AG
                    {
                        ...
                        write_Int_FRAM(CONTROL_PORTflagsaddress,PORT_CONTROL.control);	//store flag in FRAM`
                        S1.statusflags.BT_Timer=0;                              //clear the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
                    ...
                    if (buffer[1]==capT &&buffer[2] == capE && buffer[3] == cr) //BTE<CR> received  REV AG
                    {
                        ...
                        write_Int_FRAM(CONTROL_PORTflagsaddress,PORT_CONTROL.control);	//store flag in FRAM 
                        S1.statusflags.BT_Timer=1;                              //set the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
            ...
            (channel>=17&&(MUX4_ENABLE.mflags.mux16_4==1)) |        //16 channel
                ...
                case capL:
                    ...
                    if (buffer[1] == capD) //LD received
                    {
                        if (LC2CONTROL.flags.LogInterval) {
                            ...
                            write_Int_FRAM(LC2CONTROLflagsaddress,LC2CONTROL.full);	//store flag in FRAM  
                            S1.statusflags.Logint=0;                            //Clear the MODBUS status flag    REV BF
                            write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
                            break;
                        }
                        crlf();
                    ...
                    if (buffer[1] == capE) //LE received
                    {
                        ...
                        if (!LC2CONTROL.flags.LogInterval) //is Log interval flag clear?
                        {
                            ...
                            S1.statusflags.Logint=1;                            //Set the MODBUS status flag    REV BF
                            write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
                ...
                case capM:
                    ...
                    if (buffer[1] == capX && buffer[2] == one && buffer[3] == cr) //Select single channel VW Datalogger
                    {
                        ...
                        S1.statusflags.CFG=Single;                              //set the MODBUS status flags    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);        
                    ...
                    if (buffer[1] == capX && buffer[2] == four && buffer[3] == cr) //Select 4 channel mux
                    {
                        ...
                        write_Int_FRAM(MUX4_ENABLEflagsaddress,MUX4_ENABLE.mux);	//store flag in FRAM  
                        S1.statusflags.CFG=VW4;                                 //set the MODBUS status flags    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);      
                    ...
                    if (buffer[1] == capX && buffer[2] == eight && buffer[3] == capT && buffer[4] == cr) //Select 8 channel thermistor mux
                    {
                        ...
                        write_Int_FRAM(MUX4_ENABLEflagsaddress,MUX4_ENABLE.mux);	//store flag in FRAM 
                        S1.statusflags.CFG=TH8;                                 //set the MODBUS status flags    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                              
                    ...
                    if (buffer[1] == capX && buffer[2] == three && buffer[3] == two && buffer[4] == capT && buffer[5] == cr) //Select 32 channel thermistor mux
                    {
                        ...
                        write_Int_FRAM(MUX4_ENABLEflagsaddress,MUX4_ENABLE.mux);	//store flag in FRAM
                        S1.statusflags.CFG=TH32;                                //set the MODBUS status flags    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                              
                    ...
                    if (buffer[1] == capX && buffer[2] == one && buffer[3] == six && buffer[4] == cr) //Select 16 channel mux
                    {
                        ...
                        write_Int_FRAM(MUX4_ENABLEflagsaddress,MUX4_ENABLE.mux);	//store flag in FRAM 
                        S1.statusflags.CFG=VW16;                                //set the MODBUS status flags    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                              
                    ...
                    if (buffer[1] == capX && buffer[2] == eight && buffer[3] == cr) //Select 8 channel VW mux
                    {
                        ...
                        write_Int_FRAM(MUX4_ENABLEflagsaddress,MUX4_ENABLE.mux);	//store flag in FRAM 
                        S1.statusflags.CFG=VW8;                                 //set the MODBUS status flags    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                              
                    ...
                    if (buffer[1] == capX && buffer[2] == three && buffer[3] == two && buffer[4] == cr) //Select 32 channel VW mux
                    {
                        ...
                        write_Int_FRAM(MUX4_ENABLEflagsaddress,MUX4_ENABLE.mux);	//store flag in FRAM 
                        S1.statusflags.CFG=VW32;                                //set the MODBUS status flags    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                              
                ...
                case capN:
                    ...
                    if (buffer[1] == capD && buffer[2] == cr && (USB_PWR | !_232 | BT_CONNECT)) //Disable Networking    REV AF
                    {
                        crlf();
                    ...
                    if (buffer[1] == capD && buffer[2] == cr && (!USB_PWR | _232 | !BT_CONNECT) && LC2CONTROL.flags.NetEnabled) //Disable Networking not allowed    REV AF
                    { //while networked
                        crlf();
                    ...
                    if (buffer[1] == capA && (USB_PWR | !_232 | BT_CONNECT) && ((isdigit(buffer[2]) && buffer[3] == cr) |
                            (isdigit(buffer[2]) && isdigit(buffer[3]) && buffer[4] == cr) |
                            (isdigit(buffer[2]) && isdigit(buffer[3]) && isdigit(buffer[4]) && buffer[5] == cr))) //Enter Network Address REV AF
                    {
                        crlf();
                        while (BusyUART1());
                    ...
                    if (buffer[1] == capA && (!USB_PWR | _232 | !BT_CONNECT) && LC2CONTROL.flags.NetEnabled && ((isdigit(buffer[2]) && buffer[3] == cr) |
                            (isdigit(buffer[2]) && isdigit(buffer[3]) && buffer[4] == cr) |
                            (isdigit(buffer[2]) && isdigit(buffer[3]) && isdigit(buffer[4]) && buffer[5] == cr))) //Enter Network Address     REV AF
                    {
                        crlf();
                ...
                case capO:
                    ...
                    if (buffer[1] == zero && buffer[2] == cr) //O0<CR> received
                    {
                        ...
                        S1.statusflags.CP=0;                                    //clear the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
                    ...
                    if (buffer[1] == one && buffer[2] == cr) //O1<CR> received
                    {
                        ...
                        S1.statusflags.CP=1;                                    //set the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
                    ...
                    if (buffer[1] == capD && buffer[2] == cr) //OD<CR> received
                    {
                        ...
                        write_Int_FRAM(CONTROL_PORTflagsaddress,PORT_CONTROL.control);	//store flag in FRAM`
                        S1.statusflags.CP_Timer=0;                              //clear the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
                        disableAlarm(Alarm2); //disable the Alarm2 interrupt		
                        controlPortStatus(1);                                   //display control port status   REV AG
                        break;
                    ...
                    if (buffer[1] == capE && buffer[2] == cr) //OE<CR> received
                    {
                        ...
                        write_Int_FRAM(CONTROL_PORTflagsaddress,PORT_CONTROL.control);	//store flag in FRAM  
                        S1.statusflags.CP_Timer=1;                              //set the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                        
                        delay(4000);                                            //REV AE    if this delay required? TEST
                        enableAlarm(Alarm2); //enable the Alarm2 interrupt
                        controlPortStatus(1);                                   //display control port status   REV AG
                ...
                case capS:
                    ...
                    if (buffer[1] == capP && isdigit(buffer[2])) //Enter Stop Logging Time
                    {
                        ...
                        write_Int_FRAM(LC2CONTROLflagsaddress,LC2CONTROL.full);	//store flag in FRAM 
                        S1.statusflags.SP=1;                                    //set the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                              
                        ...
                        if (LC2CONTROL.flags.ERROR) {
                            ...
                            write_Int_FRAM(LC2CONTROLflagsaddress,LC2CONTROL.full);	//store flag in FRAM  
                            S1.statusflags.SP=0;                                //clear the MODBUS status flag    REV BF
                            write_Int_FRAM(MODBUS_STATUS1address,S1.status1);      
                            LC2CONTROL2.flags2.SetStopTime = 0; //clear the SetStopTime flag
                            break;
                        }
                    ...
                    if (buffer[1] == capT && buffer[2] != cr) //Enter Start Logging Time
                    {
                        ...
                        write_Int_FRAM(LC2CONTROLflagsaddress,LC2CONTROL.full);	//store flag in FRAM 
                        S1.statusflags.ST=1;                                    //set the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                          
                        ...
                        if (LC2CONTROL.flags.ERROR) {
                            ...
                            write_Int_FRAM(LC2CONTROLflagsaddress,LC2CONTROL.full);	//store flag in FRAM 
                            S1.statusflags.ST=0;                                //clear the MODBUS status flag    REV BF
                            write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                              
                    ...
                    if (buffer[1] == capR && buffer[2] == zero && buffer[3] == cr) //SR0 received
                    {
                        ...
                        DISPLAY_CONTROL.flags.Synch = 0; //clear the Synch flag
                        write_Int_FRAM(DISPLAY_CONTROLflagsaddress,DISPLAY_CONTROL.display);	//store flags in FRAM 
                        S1.statusflags.Sync=0;                                  //clear the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                        
                        putsUART1(Synchnot); //"Readings are not synchronized to the top of the hour"
                        while (BusyUART1());
                    ...
                    if (buffer[1] == capR && buffer[2] == one && buffer[3] == cr) //SR1 received
                    {
                        ...
                        write_Int_FRAM(DISPLAY_CONTROLflagsaddress,DISPLAY_CONTROL.display);	//store flags in FRAM 
                        S1.statusflags.Sync=1;                                  //set the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                                
                        putsUART1(Synch); //"Readings are synchronized to the top of the hour"
                        while (BusyUART1());
            ...
            case capU:
                ...
                case capW: //Wrap Format
                    ...
                    if (buffer[1] == capF && buffer[2] == zero && buffer[3] == cr) //stop logging when memory full
                    {
                        ...
                        write_Int_FRAM(DISPLAY_CONTROLflagsaddress,DISPLAY_CONTROL.display);	//store flags in FRAM 
                        S1.statusflags.Wrap=0;                                  //Clear the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
                    ...
                    if (buffer[1] == capF && buffer[2] == one && buffer[3] == cr) //wrap memory when full
                    {
                        ...
                        write_Int_FRAM(DISPLAY_CONTROLflagsaddress,DISPLAY_CONTROL.display);	//store flags in FRAM`
                        S1.statusflags.Wrap=1;                                  //Set the MODBUS status flag    REV BF
                        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);                        
...
void disableBT(void)                                                            //REV AD
{
    ...
    else
    {
        ...
        write_Int_FRAM(DISPLAY_CONTROLflagsaddress,DISPLAY_CONTROL.display);    //store flags in FRAM   
        S1.statusflags.BT=0;                                                    //Clear the MODBUS status flag    REV BF
        write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
...
void enableBT(void)                                                      //REV AD
{
   //TURN ON BLUETOOTH:
    ...
    write_Int_FRAM(DISPLAY_CONTROLflagsaddress,DISPLAY_CONTROL.display);        //store flags in FRAM  
    S1.statusflags.BT=1;                                                        //set the MODBUS status flag    REV BF
    write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
...
unsigned long getTxChecksum(unsigned int arraySize)                             //REV AB
{
    unsigned long accumulator=0;                                                //sum of array elements to calculate checksum
    //unsigned long accumbuffer=0;                                                //load 8 bit value into 32 bit register for accumulation
    unsigned int acc=0;                                                           //loop index

    for(acc=1;acc<arraySize;acc++)   
    {
        accumulator+=TxBinaryBUF[acc];                                          
        //accumbuffer=TxBinaryBUF[acc];                                           //load the value
        //accumulator+=accumbuffer;                                               //add it to the accumulator
    }
...
void shutdown(void) 
{
    ...
    //if(DISPLAY_CONTROL.flags.BT)                                                //REM REV AE
    //    PMD3=0xEFFF;                                                            //Enable TMR6 if BT enabled 1110 1111 1111 1111 REM REV AE
    //else                                                                        //REM REV AE
    ...
    /*REM REV AE:
    if(DISPLAY_CONTROL.flags.BT)                                                //Is Bluetooth enabled? REV AE
    {
        CLKDIVbits.FRCDIV=7;                                                    //Set FRC postscaler to 256
        __builtin_disi(0x3fff);                                                 //Disable Interrupts
        //CLOCKSWITCH
        __builtin_write_OSCCONH(7);                                             //Set the new oscillator to FRC with divide-by-N
        __builtin_write_OSCCONL(1);                                             //Request clock switch
        while(OSCCONbits.OSWEN);                                                //wait until clock switch is complete
        __builtin_disi(0x0000);                                                 //Re-enable interrupts

                                                                                //Fosc is now 28.789 Khz so Fcy = 14.395 KHz & Tcy = 69.47uS
        T6CONbits.T32=0;                                                        //setup Timer6 for 16 bit operation
        T6CONbits.TCKPS=0;                                                      //1:1 prescaler
        T6CONbits.TCS=0;                                                        //Internal Fcy
        T6CONbits.TGATE=0;                                                      //Gated time accumulation disabled
        
        while(!DataRdyUART1() | BT_CONNECT)                                     //wait until comm activity or Bluetooth connects
        {

            
            PR6=BT_OFF_TIME;                                                    //Set OFF time
            
            //Turn OFF Bluetooth:
            _BT_RESET=0;                                                        //make sure RESET is low    
            BT_BAUD=0;                                                          //make sure BT_BAUD is low          
            BT_ENABLE=0;                                                        //Powerdown Bluetooth
            T6CONbits.TON=1;                                                    //Turn on the timer
            //while(!IFS2bits.T6IF);                                              //wait till timeout   TEST REM REV AE
            while(!IFS2bits.T6IF)                                               //wait till timeout TEST REV AE
            {
                testPoint(1,1);
            }
            IFS2bits.T6IF=0;                                                    //Clear the interrupt flag 
            T6CONbits.TON=0;                                                    //Turn off the timer
           
            PR6=BT_ON_TIME;                                                     //Set ON time
        ...
        __builtin_disi(0x3fff);                                                 //Disable Interrupts
        //CLOCKSWITCH
        __builtin_write_OSCCONH(2);                                             //Restore the oscillator to HS
        __builtin_write_OSCCONL(1);                                             //Request clock switch
        while(OSCCONbits.OSWEN);                                                //wait until clock switch is complete
        __builtin_disi(0x0000);                                                 //Re-enable interrupts  Clock is now back to HS
    }
    else
    {
    */
    ...
    //if(DISPLAY_CONTROL.flags.BT)                                                //was Bluetooth previously enabled? REV AD
      //  enableBT();                                                             //yes so re-enable  REV AD
}
...
void startLogging(void) {
    ...
    if (!LC2CONTROL.flags.NetEnabled && (USB_PWR | !_232 | BT_CONNECT))         //REV AF
        T4CONbits.TON = 1; //Turn timer back on
    LC2CONTROL.flags.Logging = 1; //Set Logging flag
    LC2CONTROL.flags.LoggingStartTime = 0;
    write_Int_FRAM(LC2CONTROLflagsaddress,LC2CONTROL.full);	//store flags in FRAM 
    
    S1.statusflags.Logging=1;                                                   //Set the MODBUS status flag    REV BF
    write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
...
void stopLogging(void) 
{
    ...
    S1.statusflags.Logging=0;                                                   //Clear the MODBUS status flag    REV BF
    write_Int_FRAM(MODBUS_STATUS1address,S1.status1);
...
void take_One_Complete_Reading(unsigned char store) 
{
    ...
    if ((LC2CONTROL.flags.Monitor && (USB_PWR | !_232 | BT_CONNECT)) |
...
float take_reading(unsigned char gageType)                                      //take a reading, x is gage type    REV M
{
    ...
    if (gageType == 95) //lithium coin cell reading
    {
        ...
        SAMPLE_LITHIUM = 1; //sample the lithium battery		
        //delay(500);                                                           //500uS delay for settling	REM REV AE
        delay(2000);                                                            //REV AE

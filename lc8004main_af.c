...
//	REVISION:   af
//	CHECKSUM:	0xb85f (MPLABX ver 3.15 and XC16 ver 1.26)
//	DATA(RAM)MEM:	8424/30720   27%
//	PGM(FLASH)MEM:  152247/261888 58%
...
//	Z:\8004\LC8004main_af.c (main source: x is revision level)
...
//***************************TEST FOR 3V or 12V BATTERY*****************************************************************
    ...
    //if (!_232) //is RS232?                                                      //TEST REV AE
    //    _232SHDN = 1; //Enable RS232                                            //TEST REV AE
    //disableBT();                                                                //TEST REV AE
        ...
        while (!IFS3bits.T9IF)
        {
            ...
            switch (buffer[0]) {
                ...
                case capB:
                    ...
                    if(buffer[1]==capT && buffer[2]==cr)                        //REV AD
                    {
                        displayBT();
                        break;
                    }
                    
                    if (buffer[1] == capT && buffer[2] == zero && buffer[3] == cr)  //Deactivate Bluetooth  REV AD
                    {
                        disableBT();
                        break;
                    }
                    
                    if (buffer[1] == capT && buffer[2] == one && buffer[3] == cr)   //Activate Bluetooth  REV AD
                    {
                       enableBT();                        
                       break;
                    }
                ...
                case capD:
                    ...
                    if (isdigit(buffer[1]) && !FRAM_MEMORY.flags.memEmpty) //Enter # of arrays to display from PositionArrayPointer value
                    {
                        ...
                        //    displayTemporaryStatus(id); //display memory status with current User Position    REM REV AB
                ...
                case capE:                                                      //Shutdown  REV AE                     
                    ...
                    DISPLAY_CONTROL.flags.Shutdown = 1;
                    write_Int_FRAM(DISPLAY_CONTROLflagsaddress,DISPLAY_CONTROL.display);	//store flags in FRAM 
                    IFS3bits.T9IF = 1;                                          //set the T9IF to break out on Comm() in main()
...
//******************************************************************************                                    
            ...
            case capU:
                ...
                case pound: //prevent bus collision if networked and not shut down before	
                    ...
                    {
                        crlf();
                        IFS3bits.T9IF = 1; //set the T5IF to break out on Comm() in main()
...
void disableBT(void)                                                            //REV AD
{
    ...
    else
    {
        ...
        putsUART1(BTDisabled);                                                  //Display "Bluetooth Disabled."    
        while(BusyUART1());
        Nop();
    }
}
...
void displayReading(int ch, unsigned long outputPosition) //display readings stored in external FRAM at FRAMaddress
{
    ...
    if (LC2CONTROL.flags.ID && !LC2CONTROL2.flags2.d)                      //Only display ID if ascii transmission REV AA
    {
    ...
    {
        TxBinaryBUF[arrayIDX]=year;                                             //load year into buffer[4]   REV AB
...
*/
    //REV AA:
    if (!LC2CONTROL2.flags2.d)                                                  //month,day
    {
        if(LC2CONTROL.flags.DateFormat)
        {
            month = toMonthDay(julian, year, 1);                                //get the month from the julian date
            day = toMonthDay(julian, year, 0);                                  //get the day from the julian date
            sprintf(BUF, "%d", month);                                          //format the month data ASCII
    ...
    //HOUR
    if (!LC2CONTROL2.flags2.d)                                                  //REV AA
    {
...
void enableBT(void)                                                      //REV AD
{
   //TURN ON BLUETOOTH:
    ...
    crlf();
    putsUART1(BTEnabled);                                                      //Display "Bluetooth Disabled."    
    while(BusyUART1());    
}
...
void __attribute__((__interrupt__)) _INT1Interrupt(void)                        //This is the RTC ISR when time to read occurs  REV B
{
    ...
    if (validRTC) //if valid interrupt
    {
        ...
        if (PORT_CONTROL.flags.Alarm1Enabled && (intSource == 0x01 | intSource == 0x03)) //Alarm 1 (NO NEED TO TEST Alarm1Enabled flag)
        {
        ...
        if (PORT_CONTROL.flags.Alarm2Enabled && (intSource == 0x02 | intSource == 0x03)) //Alarm 2   (NO NEED TO TEST Alarm2Enabled flag)
        {
            disableAlarm(Alarm2);

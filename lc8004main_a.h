#ifndef LC8004main_a_H
#define LC8004main_a_H
...
void __attribute__((__interrupt__)) _INT2Interrupt(void);	//This is the RTC ISR
...
typedef struct{
	...
	unsigned interrupt:1;                           //0=no INT2 interrupt occured,1=INT2 interrupt occured
...
typedef struct{
	unsigned	mux16_4:3;			//0=4 channel mux selected  (VW4)VER 6.0.7
                                    ...
                                    //5=32 channe TH mux selected (TH32)
...
cflags	PORT_CONTROL;                                   //delare instance of cflags
//*********************************************************************
...
int CaptureFlag=0;
//int FRAMTest=0;                     REM VER 6.0.13
int gageType=0;	
int NAdata=0;						//network address value for display		
int netTest=0;			
int stopTimeTest;

const float tcy=0.000000271267361;                      //3.6864MHz Fcy
const float fudgeFactor=17.70;
const float mul3V=4.55401;                              //VER 6.0.2
const float mul12V=9.06452;                             //VER 6.0.2
const float mullith3V=1.4;                              //VER 6.0.2
const float Vref=2.5;                                   //VER 6.0.3

static unsigned int tempW0;				//temporary W Registers
static unsigned int tempW1;
static unsigned int tempW2;
static unsigned int tempW3;
static unsigned int tempW4;
static unsigned int tempW5;
static unsigned int tempW6;
static unsigned int tempW7;
static unsigned int tempW8;
static unsigned int tempW9;
static unsigned int tempW10;
static unsigned int tempW11;
static unsigned int tempW12;
static unsigned int tempW13;
static unsigned int tempW14;
static unsigned int tempW15;
...
//      CFG Memory (configuration) from 0x7f820 to 0x7fe61                      //1601 bytes CFG
...
#define N2G                     PORTCbits.RC2       //H-Bridge drive            (OUTPUT)
#define _P1G                    PORTCbits.RC3       //H-Bridge drive            (OUTPUT)
#define _P2G                    PORTCbits.RC4       //H-Bridge drive            (OUTPUT)
...
//PortE Defines
#define E                       LATEbits.LATE0      //LPF Fco Select control                (OUTPUT)
#define F                       LATEbits.LATE1      //LPF Fco Select control                (OUTPUT)
#define G                       LATEbits.LATE2      //LPF Fco Select control                (OUTPUT)
#define H                       LATEbits.LATE3      //LPF Fco Select control                (OUTPUT)
...
//sector size (bytes)
#define SingleVWBytes           18                                              //21330 readings max
#define VW4Bytes                36                                              //10666 readings max
#define VW8Bytes                50                                              //7680 readings max
#define VW16Bytes               108                                             //3555 readings max
#define VW32Bytes               140                                             //2740 readings max
#define TH8Bytes                28                                              //13714 readings max
#define TH32Bytes               76                                              //5052 readings max
...
//char MXselect[]={"MX#		       Select Multiplexer Configuration(1,4,8T,16)"};  //REM VER 6.0.9
...
void clockThMux(unsigned int);                                                  //VER 6.0.6
void comm(void);
...
//void disableCOM(void);                                                        REM VER 6.0.2
void disableINT2(void);
void displayBCD(void);
void displayEEPROMError(int);                 //REM VER 6.0.0
...
//void enableCOM(void);                                                         REM VER 6.0.2
void enableINT2(void);
void formatandDisplayGageInfo(float TEMPVAL);
//void Fout(unsigned char gageType);                                            REM VER 6.0.10
...
int take_analog_reading(unsigned char);
//void take_One_Complete_Reading(void);                                         REM VER 6.0.13
//void take_One_Complete_Temp_Reading(void);                                    REM VER 6.0.13
void take_One_Complete_Reading(unsigned char);                                  //VER 6.0.13
float take_reading(unsigned char);
void testReset(void);
void testRx(void);              //VER 6.0.2
int testStopTime(void);
void testPoint(unsigned char, unsigned char);                                 
void testPoint2(unsigned char);                                  //VER 6.0.7

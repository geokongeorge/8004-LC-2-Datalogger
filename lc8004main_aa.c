...
//	REVISION:   aa
//	CHECKSUM:	0xf75c (MPLABX ver 3.15 and XC16 ver 1.26)
//	DATA(RAM)MEM:	7892/30720   26%
//	PGM(FLASH)MEM:  148452/261888 57%
...
//                                  Debug communications sometimes unable to trigger wake from sleep
//
//
//
//				VW GAGE TYPES				
...
//	Z:\8004\LC8004main_aa.c (main source: x is revision level)
...
void displayReading(int ch, unsigned long outputPosition) //display readings stored in external FRAM at FRAMaddress
{
    ...
    if (MUX4_ENABLE.mflags.mux16_4 == Single) //Single Channel VW
    {
        ...
        NOB=22;                                                                 //22 bytes for binary download
    }
...
*/
    ...
    else                                                                        
    {
        ...
        putcUART1(MSB);                                                         //mainBat MSB Binary    REV AA
        while(BusyUART1());
        putcUART1(MMSB);                                                        //mainBat MMSB Binary   REV AA
        while(BusyUART1());
        putcUART1(MMMSB);                                                       //mainBat MMMSB Binary  REV AA
        while(BusyUART1());
        putcUART1(LSB);                                                         //mainBat LSB Binary    REV AA
        while(BusyUART1());
    }
    ...
    else                                                                      
    {
        ...
        read_Flt_FRAM(scratchaddress,3);                                        //read back to load MSB,MMSB,MMMSB & LSB    REV AA
        putcUART1(MSB);                                                         //intThermProcessed MSB Binary    REV AA
        while(BusyUART1());
        putcUART1(MMSB);                                                        //intThermProcessed MMSB Binary   REV AA
        while(BusyUART1());
        putcUART1(MMMSB);                                                       //intThermProcessed MMMSB Binary  REV AA
        while(BusyUART1());
        putcUART1(LSB);                                                         //intThermProcessed LSB Binary    REV AA
        while(BusyUART1());
    }
    ...
    {
        putcUART1(etx);                                                         //transmit etx char as seperator
        while(BusyUART1());
    }

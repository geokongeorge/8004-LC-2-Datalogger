...
//	REVISION:   m
//	CHECKSUM:	0xfbe9 (MPLABX ver 3.15 and XC16 ver 1.26)
//	DATA(RAM)MEM:	7954/30720   26%
//	PGM(FLASH)MEM:  142194/261888 54%
...
void configUARTnormal(void) {
    ...
    ConfigIntUART1(UART_RX_INT_EN & //configure the UART interrupt              //REV D            
...
float getFrequency(void)
{
	...
	for(i=0;i<loopsTotal;i++)
	{
		...
		while(!VW100 && !IFS0bits.T1IF);                                        //wait while VW(100) is low
        //INTCON1bits.NSTDIS=1;                                                   //disable interrupt nesting TEST REV M
		...
		if(!IFS0bits.T1IF)                                                      //Capture frequency				
		{
			...
			while(!IFS0bits.T1IF && CaptureFlag==0)
			{	

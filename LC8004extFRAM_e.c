...
#include "LC8004extFRAM_e.h"                                                    //external FRAM function prototypes
...
char TEST=0;
unsigned int config2read=0x05;                                                      //d64 value for I2C1BRG at 925KHz      
//unsigned int config2=0x06;                                                      //d64 value for I2C1BRG at 800KHz       
...
unsigned int read_Int_FRAM(unsigned long absAdd)
{
    ...
    //if(tempdata.ui==0xFFFF && TEST==0)                                        //if memory is blank	TEST REM REV C
        //return 0;                                                             //TEST REM REV C
    //testPoint(1,1);
    //write_Int_FRAM(absAdd, tempdata.ui);                                        //Refresh   REV D REM REV E
...
//
//***************************************************************************
//void write_longFRAM(long l, unsigned int add)                                 //REM REV C
void write_longFRAM(long l, unsigned long add)                                   //REV C
{
...
void read_Flt_FRAM(unsigned long absAdd, unsigned int x)
{
    ...
    //testPoint(1,2);
    //write_Flt_FRAM(absAdd, tempdata.g);                                         //Refresh memory    REV D REM REV E
    //StopshutdownTimer();							//stop the reset timer

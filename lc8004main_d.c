...
//	REVISION:   d
//	CHECKSUM:	0x5982 (MPLABX ver 3.15 and XC16 ver 1.26)
//	DATA(RAM)MEM:	7234/30720   24%
//	PGM(FLASH)MEM:  133485/261888 51%
...
//      d       09/12/16            Incorporate RTC functions in main()
...
FRAMTest=write_intFRAM(LC2CONTROL2flagsaddress,LC2CONTROL2.full2);	//store flag in FRAM
    ...
    while (1) //infinite loop
    {
        ...
        if(DISPLAY_CONTROL.flags.Shutdown)                                      //REV D
        {
...
void shutdown(void) 
{
    ...
    sleepFRAM();                                                                //REV D
    setADCsleep();                                                              //REV D
    PMD1=0xFFDF;                                                                //shutdown modules except UART1
    PMD2=0xFFFF;
    PMD3=0xFFFF;
...
void __attribute__((__interrupt__)) _INT1Interrupt(void)                        //This is the RTC ISR when time to read occurs  REV B
{
    ...
    if (validRTC) //if valid interrupt
    {
        ...
        if (PORT_CONTROL.flags.Alarm2Enabled && (intSource == 0x02 | intSource == 0x03)) //Alarm 2
        {
            ...
            enableAlarm(Alarm2);
            //if (!PORT_CONTROL.flags.Alarm1Enabled)                            REM REV D
              //  shutdown();                                                   REM REV D

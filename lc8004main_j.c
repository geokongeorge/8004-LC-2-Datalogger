...
//	REVISION:   j
//	CHECKSUM:	0x3b55 (MPLABX ver 3.15 and XC16 ver 1.26)
//	DATA(RAM)MEM:	7940/30720   26%
//	PGM(FLASH)MEM:  142155/261888 54%
...
void displayGageInfo(int channel) //display the gage information
{
	...
	//calculate indexed address for gage type                                   //REV J

	if(!MUX4_ENABLE.mflags.mux16_4)                                             //if 4 channel MUX
	{
		address=CH1GTaddress+(0x1A*(channel-1));
		Thermaddress=_4CHMuxCH1THaddress+(0x02*(channel-1));                                 
	}

	else                                                                        //if 16 channel MUX
	{
		address=CH1GTaddress+(0x1A*(channel-1));
		Thermaddress=_4CHMuxCH1THaddress+(0x02*(channel-1));		
	}
...
void displayTempReading(void) {
    ...
    unsigned int displayChannel = 0;
    unsigned int maxchannelplusone;
    unsigned int TempFRAMaddress = XmemStart; //internal FRAM address for beginning of data
    float mainBat = 0.0;
    float intThermRaw = 0.0;
    float intThermProcessed = 0.0;
    float extThermRaw = 0.0;
    ...
    IEC1bits.INT1IE = 1; //Enable INT2
    intThermRaw = ((Vref * intThermreading) / 4096); //convert to voltage    VER 6.0.0
    intThermProcessed = V_HT2C(intThermRaw,0); //convert to degrees C           REV J
...
void enableVWchannel(unsigned char gageType)                                    //REV H
{
    ...
    //FOR TEST ONLY
            while (!DataRdyUART1() )
            {
                ...
                for(g=0;g<256;g++)
                {
                    write_AD5241(g);                                            //Set initial gain
                    delay(100000);
                }
            }
...
int take_analog_reading(unsigned char gageType) {
    ...
    if (gageType >= 85 && gageType <= 97) 
    {
        ...
        {
            Nop();                                                              //get the temp reading from the DS3231 RTC
            return analog;
        }
    ...
    for (ch = 1; ch <= ch_max; ch++) //mux loop
    {
        ...
        if (MUX4_ENABLE.mflags.mux16_4 != TH8 && MUX4_ENABLE.mflags.mux16_4 != TH32) //VW logger
        {
            if (gageType > 0 && gageType <= 6) //VW                             //REV J
            {
                ...
                if (!store)
                    IEC1bits.INT1IE = 0; //temporarily disable the INT2 interrupt
                WDTSWEnable; //Start WDT TEST LC2MUX                          //TEST REM REV E
                        ...
                        MUX4_ENABLE.mflags.mux16_4 == VW16)
                {
                    ...
                    extThermRaw=((2.5*extThermreading)/4096);	//convert to float voltage	 REV J
...
float V_HT2C(float V, unsigned int ch)                                          //REV J
{
	...
	double LogRht=0.0;
	float T32bit=0.0;
    unsigned int Thermtypeaddress=0;                                            //REV J
	...
	{
		if(MUX4_ENABLE.mflags.mux16_4)			//16 channel mux selected
			Thermtypeaddress=_16CHMuxCH1THaddress + (2*(ch-1));
		else									//4 channel mux selected

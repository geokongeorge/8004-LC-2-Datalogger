...
//	REVISION:   cg
//	CHECKSUM:	0xfd54 (MPLABX ver 3.15 and XC16 ver 1.26)
//	DATA(RAM)MEM:	8636/30720   28%
//	PGM(FLASH)MEM:  152058/261888 58%
...
//      cg      5/12/17             Add MODBUS Clock Read and Clock Write
//
//
//
//				VW GAGE TYPES				
...
//	Z:\8004\LC8004main_cg.c (main source: x is revision level)
//	Z:\8004\LC8004delay_b.c
//	Z:\8004\LC8004extFRAM_i.c
//  Z:\8004\AD5241a.c
//  Z:\8004\MODBUSh.c                                                           REV BH

//	Header Files:
//#include "p33FJ256GP710A.h"
//#include "LC8004extFRAM_i.h"                              
//#include "LC8004main_cg.h"
//#include "LC8004delay_b.h"
//#include "AD5241a.h"
//#include "MODBUSh.h"                                                          REV BH
//#include "FRAM_ADDRESSd.h                                                     REV CG
...
void READ_TIME(void)                                                            //REV CG
{
    ...
    if(LC2CONTROL2.flags2.Modbus)
    {
        RTCvalue=RTCdata;                                                       //cast to 16 bit
        write_Int_FRAM(RTCMonths,RTCvalue);                                     //store months in FRAM  
    }
    else
    {
        displayClock(RTCdata);                                                  //display it
    ...
    {
        RTCvalue=RTCdata;                                                       //cast to 16 bit
        write_Int_FRAM(RTCDays,RTCvalue);                                       //store days in FRAM
    }
    else
    {
        displayClock(RTCdata);                                                  //display it
    ...
    {
        RTCvalue=RTCdata;
        write_Int_FRAM(RTCYears,RTCvalue);
    }
    else
    {
        displayClock(RTCdata); //display it
    ...
    RTCdata = readClock(RTCHoursAddress);                                       //get the hours from the RTC
    if(LC2CONTROL2.flags2.Modbus)
    {
        RTCvalue=RTCdata;
        write_Int_FRAM(RTCHours,RTCvalue);
    }
    else
    {
        displayClock(RTCdata);                                                  //display it
    ...
    RTCdata = readClock(RTCMinutesAddress);                                     //get the minutes from the RTC
    if(LC2CONTROL2.flags2.Modbus)
    {
        RTCvalue=RTCdata;
        write_Int_FRAM(RTCMinutes,RTCvalue);
    }
    else
    {
        displayClock(RTCdata);                                                  //display it
    ...
    RTCdata = readClock(RTCSecondsAddress);                                     //get the seconds from the RTC
    if(LC2CONTROL2.flags2.Modbus)
    {
        RTCvalue=RTCdata;
        write_Int_FRAM(RTCSeconds,RTCvalue);
    }
    else
    {
        displayClock(RTCdata);                                                  //display it    
...
void take_One_Complete_Reading(unsigned char store) 
{
    ...
    if ((!LC2CONTROL2.flags2.Modbus && LC2CONTROL.flags.Monitor && (USB_PWR | !_232 | BT_CONNECT)) |
            (!LC2CONTROL2.flags2.Modbus && LC2CONTROL.flags.Monitor && (!USB_PWR | _232 | !BT_CONNECT) && LC2CONTROL.flags.NetEnabled && netTest == 1)) //REV CE
    {
        IEC1bits.INT1IE = 0; //temporarily disable the INT1 interrupt
...
void WRITE_TIME(void)                                                           //REV CG
{
    ...
    clockdata=read_Int_FRAM(RTCYears);                                          //get the years
    RTCyears=clockdata;
    setClock(RTCYearsAddress, RTCyears);                                        //load it into the RTC
    
    clockdata=read_Int_FRAM(RTCMonths);                                         //get the months
    RTCmonths=clockdata;
    setClock(RTCMonthsAddress, RTCmonths);                                      //load it into the RTC
    
    clockdata=read_Int_FRAM(RTCDays);                                           //get the days
    RTCdays=clockdata;
    setClock(RTCDaysAddress, RTCdays);                                          //load it into the RTC

    clockdata=read_Int_FRAM(RTCHours);                                          //get the hours
    RTChours=clockdata;
    setClock(RTCHoursAddress, RTChours);                                        //load it into the RTC
    
    clockdata=read_Int_FRAM(RTCMinutes);                                        //get the minutes
    RTCminutes=clockdata;
    setClock(RTCMinutesAddress, RTCminutes);                                    //load it into the RTC
    
    clockdata=read_Int_FRAM(RTCSeconds);                                        //get the seconds

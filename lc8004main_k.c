...
//	REVISION:   k
//	CHECKSUM:	0xc8f9 (MPLABX ver 3.15 and XC16 ver 1.26)
//	DATA(RAM)MEM:	7942/30720   26%
//	PGM(FLASH)MEM:  142056/261888 54%
...
//                                  debug external temperature reading
//
//
//
//
...
void displayTempReading(void) {
    ...
    for (displayChannel = 1; displayChannel < maxchannelplusone; displayChannel++) {
        ...
        if (MUX4_ENABLE.mflags.mux16_4 == Single | //VER 6.0.13
            ...
            //TempFRAMaddress = 0xF50E + (6 * (displayChannel - 1)); //compute the reading address  REM REV K
            TempFRAMaddress = XmemStart + (6 * (displayChannel - 1)); //compute the reading address    REV K

        if (MUX4_ENABLE.mflags.mux16_4 == VW8 | MUX4_ENABLE.mflags.mux16_4 == VW32) //VER 6.0.13
            //TempFRAMaddress = 0xF50C + (4 * (displayChannel - 1));    REM REV K
        ...
        {
            //TempFRAMaddress = 0xF50C + (2 * (displayChannel - 1)); //compute the reading address   VER 6.0.7  REM REV K
    ...
    for (ch = 1; ch <= ch_max; ch++) //mux loop
    {
        ...
        if (MUX4_ENABLE.mflags.mux16_4 != TH8 && MUX4_ENABLE.mflags.mux16_4 != TH32) //VW logger
        {
            if (gageType > 0 && gageType <= 6) //VW                             //REV J
            {
                ...
                //VWreading = take_reading(gageType); //take VW reading (or other gage type)    TEST REM REV K
